...
export type OperatorSubmission = {
  ...
  aiReview: { decision?: ReviewDecision; confidence?: number; reason?: string; description?: string; proposed_tags?: string[]; concerns?: string[] } | null;
  photoUrl: string | null;
  photoPath: string | null;
};

export type OperatorDashboard = {
  stats: { published: number; candidateLeads: number; aiReady: number; humanReview: number; gptRejected: number; reviewed: number };
  queue: OperatorSubmission[];
...
export const submitOperatorReview = (entityType: OperatorSubmission['entityType'], id: string, action: 'approve' | 'reject') => request('/api/operator/review', { method: 'POST', body: JSON.stringify({ entityType, id, action }) }) as Promise<{ ok: boolean; message: string }>;
